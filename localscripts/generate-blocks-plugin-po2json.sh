...
# Engaging networks strings
pofilenames=$(ls translations/planet4-plugin-gutenberg-blocks/languages/enform/*.po)
for pofilename in $pofilenames
do
   text_domain='planet4-engagingnetworks-backend'
   if [[ "$pofilename" =~ .*"$text_domain".* ]]; then
     suffix='script'
   else
     suffix='frontend'
